...
from config import BASE_DIR, ensure_project_structure, validate_config
...
async def lifespan(fastapi_app: FastAPI):
    ...
    from core.auth import ensure_admin_exists, init_auth_db
    from core.db_config_manager import init_config_db, load_config_to_memory, seed_initial_config
    init_auth_db()
    ensure_admin_exists()
    init_config_db()
    seed_initial_config()
    ...
    # ─── Ejecutar Bundler de Frontend ───
    try:
        from scripts.bundler import bundle_js
        bundle_js()
    except Exception as e:
        logger.error(f"Error ejecutando el bundler de JS en el inicio: {e}")
    ...
    async def load_snapshots_async():
        ...
        try:
            ...
            if vl_snap:
                cache.set_cache("/analytics/deliveries", vl_snap)
                logger.info(">>> Snapshot Entregas restaurado exitosamente.")
            if inventory_snap:
                cache.set_cache("/analytics/inventory", inventory_snap)
                logger.info(">>> Snapshot Movimientos restaurado exitosamente.")
            if ots_snap:
                cache.set_cache("/analytics/ots", ots_snap)
    ...
    from services.background_tasks import refresh_analytics
...
def initialize_app(fastapi_app: FastAPI) -> None:
    ...
    try:
        ...
        if not os.path.exists(_static_path):
            ...
            logger.info("Recursos estáticos montados correctamente.")
