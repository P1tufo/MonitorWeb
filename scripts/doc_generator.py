...
def load_gitignore(root_dir):
    ignore_dirs = {".git", "__pycache__", "htmlcov", "graphify-out", "venv", "node_modules", "PDFs_Generados", "Temp_Assets", "scratch", "_legacy_reference", ".pytest_cache", CACHE_DIR_NAME}
...
from scripts.doc_prompts import (
    PROMPT_AUDIT_SYSTEM,
    PROMPT_AUDIT_USER,
    ...
    for attempt in range(retries):
        try:
            ...
            if not response_content.strip():
                tqdm.write("   [!] Advertencia: La IA devolvió una respuesta vacía.")
...
def document_file(filepath, current, total):
    ...
    try:
        ...
        if 'cache_path' in locals() and os.path.exists(cache_path):
            os.remove(cache_path)
        if 'cache_path_mejora' in locals() and os.path.exists(cache_path_mejora):
            os.remove(cache_path_mejora)
        return False
...
def analyze_structure_pre_flight():
    ...
    # Guardar mapa actual
    with open(tree_cache_path, 'w', encoding='utf-8') as tf:
        tf.write(tree_map)
...
def compile_master_file(files_ordered):
    ...
    os.path.join(CACHE_DIR, "project_tree.md")
    analysis_cache_path = os.path.join(CACHE_DIR, "analisis_estructural.md")
    os.path.join(CACHE_DIR, "auditoria_global.md")

    with open(OUTPUT_FILE, 'w', encoding='utf-8') as master:
        master.write("# Documentación Técnica Global - MonitorWeb\n")
        ...
        for filepath in files_ordered:
            ...
            if os.path.exists(cache_path_mejora):
                with open(cache_path_mejora, 'r', encoding='utf-8') as cf:
                    ...
                    master_imp.write("\n---\n\n")
    print("✓ Compilación de mejoras finalizada.")
...
def compile_by_folders(files_ordered):
    ...
    if os.path.exists("docs"):
        for root_dir, _dirs, files_in_dir in os.walk("docs", topdown=False):
            if root_dir == "docs":
                continue
...
def prepare_model():
    ...
    try:
        ...
        if MODEL_NAME not in existing_models and (MODEL_NAME + ":latest") not in existing_models:
            ...
            print("[*] Iniciando descarga automática. Esto puede tardar unos minutos...")
            ollama.pull(MODEL_NAME)
            print(f"✓ Modelo '{MODEL_NAME}' descargado con éxito.")
        else:
            print("✓ Motor de IA listo y modelo verificado.")

    except Exception as e:
        print("\n[X] ERROR CRÍTICO: No se pudo conectar con Ollama.")
        print("    Asegúrate de que Ollama esté abierto y funcionando.")
        print(f"    Detalle: {e}")
        sys.exit(1)
