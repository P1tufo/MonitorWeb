...
async def analytics_tasks_api(user = Depends(get_current_user), session: Session = Depends(get_session_dep), cache: CacheManager = Depends(get_cache_manager), sync: SyncStateManager = Depends(get_sync_manager)):
    """API JSON para analíticas de Tareas (Warehouse Tasks)."""

    # Intentar recuperar de caché (limpiado de request/user)
    cached_ctx = cache.get_cache("/api/v1/analytics/tasks")
    if cached_ctx:
        return AnalyticsTasksResponse(data=cached_ctx, is_syncing=sync.is_syncing)
    ...
    try:
        ...
        cache.set_cache("/api/v1/analytics/tasks", clean_context.copy())
        return AnalyticsTasksResponse(data=clean_context, is_syncing=sync.is_syncing)
