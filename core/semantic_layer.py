    ...
    "movimientos": Dataset(
        ...
        metrics=[
            ...
            Metric(
                ...
                format="percent",
                is_complex_formula=True,
                formula_template="ROUND(SUM(CASE WHEN {col} LIKE '%Ingreso%' AND IFNULL({table}.texto_cab_documento, '') NOT GLOB '*PGP*' AND IFNULL({table}.referencia, '') NOT GLOB '*PGP*' AND NOT (TRIM({table}.cmv) IN ('201', '221') AND (LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%mes%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%mes%')) THEN 100.0 ELSE 0.0 END) / NULLIF(SUM(CASE WHEN ({col} LIKE '%Centro Costo%' OR {col} LIKE '%Orden/Reserva%') AND IFNULL({table}.texto_cab_documento, '') NOT GLOB '*PGP*' AND IFNULL({table}.referencia, '') NOT GLOB '*PGP*' AND NOT (TRIM({table}.cmv) IN ('201', '221') AND (LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%mes%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%mes%')) THEN 1.0 ELSE 0.0 END), 0), 1)"
            ),
            Metric(
                id="met_return_rate",
            ...
            Metric(
                ...
                format="percent",
                is_complex_formula=True,
                formula_template="ROUND(SUM(CASE WHEN TRIM({col}) IN ('201', '261', '221') AND IFNULL({table}.texto_cab_documento, '') NOT GLOB '*PGP*' AND IFNULL({table}.referencia, '') NOT GLOB '*PGP*' AND NOT (TRIM({col}) IN ('201', '221') AND (LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%mes%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%mes%')) AND NOT ((TRIM({col}) = '201' AND (IFNULL({table}.referencia, '') GLOB '*81[0-9][0-9][0-9][0-9][0-9][0-9][0-9]*' OR IFNULL({table}.referencia, '') GLOB '*081[0-9][0-9][0-9][0-9][0-9][0-9][0-9]*' OR IFNULL({table}.texto_cab_documento, '') GLOB '*81[0-9][0-9][0-9][0-9][0-9][0-9][0-9]*' OR IFNULL({table}.texto_cab_documento, '') GLOB '*081[0-9][0-9][0-9][0-9][0-9][0-9][0-9]*')) OR (TRIM({col}) = '261' AND ((IFNULL({table}.referencia, '') = '' AND IFNULL({table}.texto_cab_documento, '') = '') OR IFNULL({table}.texto_cab_documento, '') GLOB '*PGE*' OR IFNULL({table}.referencia, '') GLOB '*PGE*'))) THEN 100.0 ELSE 0.0 END) / NULLIF(SUM(CASE WHEN TRIM({col}) IN ('201', '261', '221') AND IFNULL({table}.texto_cab_documento, '') NOT GLOB '*PGP*' AND IFNULL({table}.referencia, '') NOT GLOB '*PGP*' AND NOT (TRIM({col}) IN ('201', '221') AND (LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.texto_cab_documento, '')) LIKE '%mes%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%cierre%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%dev%' OR LOWER(IFNULL({table}.referencia, '')) LIKE '%mes%')) THEN 1.0 ELSE 0.0 END), 0), 1)"
            ),
            Metric(
