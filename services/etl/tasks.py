...
class WarehouseTaskAdapter(BaseWMSProcessor):
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df_cols:
            ...
            elif base_name in mapping:
                new_cols.append(mapping[base_name])
            else:
                if "OT" in c:
                    new_cols.append("numero_ot")
                elif "Pos" in c:
                    new_cols.append("pos")
                elif "Ctd" in c:
                    new_cols.append("ctd_teor_dsd")
                else:
                    new_cols.append(c.lower().replace('.', '').replace(' ', '_'))
